...
public class RelationshipSquareAdapter extends RelationshipBaseAdapter implements OnClickListener{
	public RelationshipSquareAdapter(Activity context,List<DynamicItem> data,Handler handler){
...
//				viewCache.getmFromTV().setText("");
			...
			if (item.getSubCommentData().size() > 0 && item.isOpen()) {
				...
				for (int i = item.getSubCommentData().size()-1; i >=0; i--) {
					...
					tView.setTag(R.id.relation_circle_message_lv,Integer.valueOf(1));
	    ...
	    viewCache.getmContentTV().setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				String text=((TextView)v).getText().toString();
                 cm.setText(text);
                 Toast.makeText(mContext, "已经将 '"+text+"' 复制到剪贴板中!", 0).show();
                 return true;
...
//						}
//						RelationshipSquareAdapter.this.notifyDataSetChanged();
//						
//					}
//					
//					@Override
...
//												Contants.SUPPORT.NO);
//									}
//									RelationshipSquareAdapter.this
...
//				support(item.getAid(), position , 0, 1);
...
//				support(item.getAid(), position , 1, 1);
				Message msg = mHandler.obtainMessage();
				msg.what = 4;
				msg.arg1 = position;
				msg.arg2 = item.getAid();
...
//				getComments(item.getAid(), position, 1);
				Message msg = mHandler.obtainMessage();
				msg.what = 5;
				msg.arg1 = position;
				msg.arg2 = item.getAid();
		...
		case R.id.relationship_circle_item_comment_btn:
			...
			msg2.what = 1;
			msg2.arg1 = position;
			msg2.arg2 = this.getCount();//点击评论时ListView的Item总量
			msg2.obj = null;
...
//			showSelector(item);
	 ...
	 * @param item
	 */
//    public void showSelector(final DynamicItem item) {
//        try {
...
//					public void onSuccess(int statusCode, String content) {
//						super.onSuccess(statusCode, content);
//						log.i("square  getComments result = " + content);
//						if (TextUtils.isEmpty(content)) {
...
//					msg.what = 1;
//					msg.arg1 = pos;
//					msg.arg2 = RelationshipSquareAdapter.this.getCount();
//					msg.obj = reItem;
