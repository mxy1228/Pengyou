...
public class FriendInfoAdapter extends RelationshipBaseAdapter implements OnClickListener{

	public FriendInfoAdapter(Activity context,List<DynamicItem> data,Handler handler){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		FriendInfoViewCache  viewCache = null;
		if(convertView == null){
			convertView = mInflater.inflate(R.layout.friendinfo_dynamic_item, null);
			viewCache = new FriendInfoViewCache(convertView);
			convertView.setTag(viewCache);
		}else{
			viewCache = (FriendInfoViewCache)convertView.getTag();
		}
		...
		if(item != null){
			...
			case Contants.DYNAMIC_TYPE.SHARE_GAME:
				...
				viewCache.getmTypeTV().setVisibility(View.VISIBLE);
				viewCache.getmTypeTV().setText(mContext.getString(R.string.recomend_game_tag));

				if(item.getGame() != null){
					viewCache.getmGameRL().setVisibility(View.VISIBLE);
					viewCache.getmGameRL().setOnClickListener(this);
					viewCache.getmGameType().setText(
							item.getGame().getGametype());
					if (item.getGame().getPlatform() == 2) {
						viewCache.getmGameIconFrom().setVisibility(View.GONE);
					} else {
						viewCache.getmGameIconFrom()
								.setVisibility(View.VISIBLE);
					}

					CYImageLoader.displayIconImage(
							item.getGame().getGameicon(),
							viewCache.getmGameIconIV(), mIconOption);
					viewCache.getmGameNameTV().setText(
							item.getGame().getGamenm());

					viewCache.getmPlayerCountTV().setVisibility(View.VISIBLE);
					viewCache.getmPlayerCountTV().setText(
							mContext.getString(R.string.play_count, item
									.getGame().getPlaynum()));
					viewCache.getmGameZoneIcon().setVisibility(View.GONE);
				}
				else{
			...
			case Contants.DYNAMIC_TYPE.PUB_TEXT:
				...
				viewCache.getmCaptureIV().setVisibility(View.GONE);
				viewCache.getmGameRL().setVisibility(View.GONE);
				viewCache.getmTypeTV().setVisibility(View.GONE);
				break;
				...
				if (item.getPicturemiddle() != null) {
					...
					if(item.getPicturemiddle().getHeight() > item.getPicturemiddle().getWidth()){
						...
						if(Config.screenWidthWithDip > 0){
							...
							params = new LayoutParams(mContext.getResources().getDimensionPixelSize(R.dimen.pic_width)
									, mContext.getResources().getDimensionPixelSize(R.dimen.pic_height));
							params.leftMargin = mContext.getResources().getDimensionPixelSize(R.dimen.dynamic_margin_left);
							params.topMargin = mContext.getResources().getDimensionPixelSize(R.dimen.padding_left_right);
							params.bottomMargin = mContext.getResources().getDimensionPixelSize(R.dimen.padding_left_right);
						    viewCache.getmCaptureIV().setLayoutParams(params);
			...
			case Contants.DYNAMIC_TYPE.MY_DYNAMIC:
				...
				if (item.getMyPicture() != null && !"".equals(item.getMyPicture())) {
					try {
						Bitmap bitmap = FileUtil.getBitmapFromFilePath(item.getMyPicture(),mContext.getResources().getDimensionPixelSize(R.dimen.publish_pic_width)
								, mContext.getResources().getDimensionPixelSize(R.dimen.publish_pic_height));
						if (bitmap != null) {
							LayoutParams params;
							...
							viewCache.getmCaptureIV().setImageBitmap(bitmap);
							viewCache.getmCaptureIV().setOnClickListener(this);
							viewCache.getmCaptureIV().setTag(position);
							viewCache.getmCaptureIV().setOnTouchListener(Util.onTouchCaptureListener);
						}else {
							LayoutParams params =new LayoutParams(mContext.getResources().getDimensionPixelSize(R.dimen.pic_height)
											, mContext.getResources().getDimensionPixelSize(R.dimen.pic_width));
									params.leftMargin = mContext.getResources().getDimensionPixelSize(R.dimen.dynamic_margin_left);
									params.topMargin = mContext.getResources().getDimensionPixelSize(R.dimen.padding_left_right);
									params.bottomMargin = mContext.getResources().getDimensionPixelSize(R.dimen.padding_left_right);
			...
			case Contants.DYNAMIC_TYPE.GAME_CIRCLE:
				if(item.getStar() != null){
					if(!item.getStar().equals("")){
						...
						viewCache.getmRelScoreRatingBar().setRating(Float.valueOf(mData.get(position).getStar()));
				...
				if (item.getGame() != null) {
					viewCache.getmGameRL().setVisibility(View.VISIBLE);
					viewCache.getmGameRL().setOnClickListener(this);
					viewCache.getmGameZoneIcon().setVisibility(View.VISIBLE);
					viewCache.getmGameType().setText(
							item.getGame().getGametype());
					if (item.getGame().getPlatform() == 2) {
						viewCache.getmGameIconFrom().setVisibility(
								View.GONE);
					} else {
						viewCache.getmGameIconFrom().setVisibility(
								View.VISIBLE);
					...
					CYImageLoader.displayIconImage(item.getGame()
							.getGameicon(), viewCache.getmGameIconIV(),
							mIconOption);
					viewCache.getmGameNameTV().setText(
							item.getGame().getGamenm());
					viewCache.getmPlayerCountTV().setVisibility(
							View.VISIBLE);
					viewCache.getmPlayerCountTV().setText(
							mContext.getString(R.string.play_count, item
									.getGame().getPlaynum()));
			...
			if (item.getText() != null && !"".equals(item.getText().trim())) {
				if(item.getText().toCharArray().length > Config.dynamicCharCountLimit){
					viewCache.getmContentALL().setTag(R.id.friendinfo_dynamic_item_content_tv, viewCache.getmContentTV());
			...
			if(viewCache.getmOpenCommentBtn().getVisibility() == View.VISIBLE){
				...
				viewCache.getmCommentTitleLayout().setTag(R.id.friendinfo_dynamic_item_comment_item_pb,viewCache.getmPB());
				viewCache.getmCommentTitleLayout().setTag(R.id.friendinfo_dynamic_item_comment_rl, viewCache.getmCommentRL());
				viewCache.getmCommentTitleLayout().setTag(R.id.friendinfo_dynamic_item_comment_seg_line,viewCache.getmOpenCommentBtn());
				viewCache.getmCommentTitleLayout().setTag(R.id.friendinfo_dynamic_item_time_tv,viewCache.getmPackUpBtn());
			}
			else{
			...
			if (item.getSubCommentData().size() > 0 && item.isOpen()) {
				...
				for (int i = item.getSubCommentData().size()-1; i >=0; i--) {
					...
					tView.setTag(R.id.relation_circle_message_lv,Integer.valueOf(3));
...
//	}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.friendinfo_dynamic_item_support_btn:
			...
			break;
		case R.id.friendinfo_dynamic_item_comment_head:
			...
			CYSystemLogUtil.behaviorLog(behaviorInfo);
			LinearLayout rl = (LinearLayout)v.getTag(R.id.friendinfo_dynamic_item_comment_rl);
			ProgressBar pb = (ProgressBar)v.getTag(R.id.friendinfo_dynamic_item_comment_item_pb);
            ((Button)v.getTag(R.id.friendinfo_dynamic_item_comment_seg_line)).setVisibility(View.GONE);
            if(rl.isShown()){
			}else{
				rl.setVisibility(View.VISIBLE);
				((Button)v.getTag(R.id.friendinfo_dynamic_item_time_tv)).setVisibility(View.VISIBLE);
				item.setOpen(true);
			...
			break;
		case R.id.friendinfo_dynamic_item_pack_up_btn:
			 behaviorInfo = new BehaviorInfo(
			...
			FriendInfoAdapter.this.notifyDataSetChanged();
			break;
		case R.id.friendinfo_dynamic_item_comment_btn:
			Message msg = mHandler.obtainMessage();
			msg.what = 1;
			msg.arg1 = position;
			msg.obj = null;
			mHandler.sendMessage(msg);
			break;
		case R.id.friendinfo_dynamic_item_game_rl:
			if(item.getGame() == null){
				Toast.makeText(mContext, R.string.failed_get_data, Toast.LENGTH_SHORT).show();
				return;
			}
			intent.setClass(mContext, GameCircleDetailActivity.class);
			intent.putExtra(Params.INTRO.GAME_CODE, item.getGame().getGamecode());
			intent.putExtra(Params.INTRO.GAME_NAME, item.getGame().getGamenm());
			...
			break;
		case R.id.friendinfo_dynamic_item_capture_iv:
			 behaviorInfo = new BehaviorInfo(
			...
			mContext.startActivity(intent);
			break;
		case R.id.friendinfo_dynamic_item_content_tv1:
			 TextView  tView = (TextView)v.getTag(R.id.friendinfo_dynamic_item_content_tv);
			if(item.isNeedPullDown()){
...
//					public void onSuccess(int statusCode, String content) {
//						super.onSuccess(statusCode, content);
//						log.i(" friendinfo getComments result = " + content);
//						if (TextUtils.isEmpty(content)) {
...
//					return;
//			   }
//			   DynamicCommentItem dyItem = (DynamicCommentItem)v.getTag(R.id.friendinfo_dynamic_item_time_pic_iv);
//			    int pos = ((Integer) v
//						.getTag(R.id.friendinfo_dynamic_item_time_tv))
//						.intValue();
