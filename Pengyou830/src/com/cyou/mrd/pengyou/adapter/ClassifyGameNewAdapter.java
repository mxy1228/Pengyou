 ...
 * 
 */
public class ClassifyGameNewAdapter extends BaseAdapter {
	...
	public ClassifyGameNewAdapter(Context context, List<GameItem> data) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		ClassifyGameNewViewCache viewCache = null;
		// try {
		...
		// }
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.classify_game_new_item,
					null);
			viewCache = new ClassifyGameNewViewCache(convertView);
			convertView.setTag(viewCache);
		} else {
			viewCache = (ClassifyGameNewViewCache) convertView.getTag();
		}
		...
		viewCache.getTxtPlayerCount().setText(
				mContext.getString(R.string.classify_game_new_gameinfo,
						Util.getDateForH(item.getCreatedate()),
		...
		btnDownload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(final View v) {
				...
				if (Util.isInstallByread(item.getIdentifier())) {// 若已安装
					...
					// Intent("android.intent.action.playgame");
					...
					if (downloadItem != null
						...
						case DownloadParam.C_STATE.DONE:
							...
							} else {// 若已经下载完成状态,但是找不到安装文件,则 删除DB记录
								Toast.makeText(
										mContext,
