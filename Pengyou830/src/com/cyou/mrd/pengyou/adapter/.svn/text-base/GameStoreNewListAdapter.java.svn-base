...
public class GameStoreNewListAdapter extends BaseAdapter {
	...
	public GameStoreNewListAdapter(Context context, List<GameItem> data,
			GridView gd) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = (LinearLayout) mInflater.inflate(
					R.layout.gamestore_newlist_item, null);
			...
			if (item == null || TextUtils.isEmpty(item.getGamecode())) {
				...
				if (!TextUtils.isEmpty(dateMsg)) {
					if (dateMsg.contains("年")) {
						int index = dateMsg.indexOf("年");
						dateMsg = dateMsg
								.substring(index + 1, dateMsg.length());
					}
					viewCache.getmTxtGameInfo().setText(dateMsg);
				}
