...
public class GameNewAdapter extends BaseAdapter {
    ...
    public GameNewAdapter(Context context, List<GameItem> data) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (!TextUtils.isEmpty(item.getGtname())) {
			...
			gtName = gtName;
			viewCache.getTxtPlayerCount().setText(mContext.getString(R.string.gamestore_new_gameinfo,Util.getDateForH(item.getCreatedate()),Util.getGameSize(item.getFullsize()), gtName));
		} else {
		    viewCache.getTxtPlayerCount().setText(mContext.getString(R.string.gamestore_new_gameinfo,Util.getDateForH(item.getCreatedate()),Util.getGameSize(item.getFullsize()),""));
		}
        try {
            ...
            public void onClick(final View v) {
                ...
                if (Util.isInstallByread(item.getIdentifier())) {// 若已安装
                    ...
                    if (downloadItem != null
                      ...
                      //用户行为统计
                        BehaviorInfo behaviorInfo = new BehaviorInfo(CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_NEWRANK_MOREGAME_GAMEDETAIL_DOWNLOAD_ID,
                                CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_NEWRANK_MOREGAME_GAMEDETAIL_DOWNLOAD_NAME);
