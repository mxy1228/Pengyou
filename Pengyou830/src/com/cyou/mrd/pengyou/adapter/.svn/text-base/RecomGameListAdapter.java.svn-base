...
public class RecomGameListAdapter extends BaseAdapter {
	...
	public RecomGameListAdapter(Context context, List<GameItem> data) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		RecomGameItemViewCache viewCache = null;
		// convertView = viewCaches.get(position);
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.recom_game_item, null);
			viewCache = new RecomGameItemViewCache(convertView);
			convertView.setTag(viewCache);
		} else {
			viewCache = (RecomGameItemViewCache) convertView.getTag();
		}
		...
		// viewCache.getTxtGameName().setText(
		// mContext.getString(R.string.recom_game_name, item.getGtname(),
		// item.getName()));
		viewCache.getTxtGameName().setText(item.getName());
		String frdplay = item.getUsrplay();
		if (TextUtils.isEmpty(frdplay)) {
			frdplay = "0";
		}
		viewCache.getTxtPlayerCount().setText(
		mContext.getString(R.string.classify_game_hot_gameinfo,
		frdplay, Util.getGameSize(item.getFullsize())));
		try {
			if (!TextUtils.isEmpty(item.getRank())) {
				viewCache.getRatingBar().setRating(
						Float.parseFloat(item.getRank()));
			}
		} catch (Exception e) {
			log.e(e);
		}
		...
		btnDownload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(final View v) {
				BehaviorInfo behaviorInfo = new BehaviorInfo(
						CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_GAME_DOWNLOAD_ID,
						CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_GAME_DOWNLOAD_NAME);
