...
public class GameDetailGuessLikeAdapter extends BaseAdapter{
	...
	private static final int MAX_GAME_NAME =9;
	private RelativeLayout mpop;
	boolean isClickMultiTimes = false;

	public GameDetailGuessLikeAdapter(Context context, List<GameItem> data,RelativeLayout rl) {
				...
				if (Util.isInstallByread(item.getIdentifier())) {// 若已安装
					...
					if (downloadItem != null
						...
						btnDownload.setTextColor(mContext.getResources()
							...
							Animation downloadanimation=AnimationUtils.loadAnimation(mContext, R.anim.settingswindow_out_anim);
							mpop.startAnimation(downloadanimation);
