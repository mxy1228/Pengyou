...
public class RelationshipCircleAdapter extends RelationshipBaseAdapter implements OnClickListener{
	public RelationshipCircleAdapter(Activity context,List<DynamicItem> data,Handler handler){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(item != null){
			...
			case Contants.DYNAMIC_TYPE.GAME_CIRCLE:
				if(item.getStar() != null && !item.getStar().equals("")){
					viewCache.getmDynamicFailBtn().setVisibility(View.GONE);
            ...
            //请求评论的时机           
			if (item.getCommentcnt() != 0 && item.isCommentUpdate()) {
//				getComments(item.getAid(), position, 0);
			...
			//double check			
			if(item.getSupportusr().size() != item.getSupportcnt()){
				item.setSupportcnt(item.getSupportusr().size());
			}
			if(item.getCommentcnt() != item.getComments().size()){
				item.setCommentcnt(item.getComments().size());
			}
			
			if(item.getSupportcnt()== 0  && ((item.getSupportusr() != null ) && (item.getSupportusr().size() == 0))){
				viewCache.getmCommentTitleLayout().setVisibility(View.GONE);
			}
			else{
				viewCache.getmCommentTitleLayout().setVisibility(View.VISIBLE);
			}
			if(item.getmSupporterStr()!=null){
				...
				spannableStringBuilder.append(item.getmSupporterStr());
			...
			if (item.getComments() != null) {
				viewCache.getmCommentRL().removeAllViews();
				for (int i = item.getComments().size() -1; i >= 0; i--) {
					TextView tView = new TextView(mContext);
					tView.setLayoutParams(LP_FW);
					tView.setTag(R.id.relation_circle_message_empty,item.getComments().get(i));
					tView.setTag(R.id.relation_circle_dot,Integer.valueOf(position));
					tView.setTag(R.id.relation_circle_message_lv,Integer.valueOf(0));
					...
					int reUid = 0;
					if(item.getComments().get(i).getReplyto()!= null){
						DynamicCommentReplyItem reply =item.getComments().get(i).getReplyto();
						reNameString = reply.getNickname();
						reUid = reply.getUid();

					}
					String  nickName = item.getComments().get(i).getNickname();
					int uid = item.getComments().get(i).getUid();
					SpannableString  spstr;
					if(reNameString != null){						
						spstr = getReplyUser(nickName, uid, reNameString, reUid, item.getComments().get(i).getText());					
						...
						spstr = getSingleUser(nickName, uid, item.getComments().get(i).getText());
						spstr.setSpan(new ClickSpan(uid), 0, nickName.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
						tView.setText(spstr);
					}					
...
//					Util.initComment(tView);
			...
			if(item.getComments().size() != 0 && item.getSupportusr().size() != 0){
		...
		viewCache.getmContentTV().setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View v) {
				String text=((TextView)v).getText().toString();
                 cm.setText(text);
                 Toast.makeText(mContext,mContext.getString(R.string.copy_text, text), Toast.LENGTH_SHORT).show();
                 return true;
...
//						RelationshipCircleAdapter.this.notifyDataSetChanged();
//						if(cancel == 0){
...
//									addToSupporterList(position, cancel);
//									RelationshipCircleAdapter.this
...
//				support(item.getAid(), position ,0,0);
				Message msg = mHandler.obtainMessage();
				msg.what = 3;
				msg.arg1 = position;
				msg.arg2 = item.getAid();
...
//				support(item.getAid(), position ,1,0);
				Message msg = mHandler.obtainMessage();
				msg.what = 4;
				msg.arg1 = position;
				msg.arg2 = item.getAid();
...
//    }
    ...
    private void republish (DynamicItem item) {
    	if (item != null) {
    		DynamicDao mDao = new DynamicDao(this.mContext);
    		DynamicInfoItem  iDynamic = mDao.getDynamicByPid(item.getPid(),UserInfoUtil.getCurrentUserId());
//    		if(iDynamic.getStatus() == Contants.SEND_DYNAMIC_STATUS.SENDING){
//    			Toast.makeText(mContext, "该动态正在发送中，请稍后", Toast.LENGTH_SHORT).show();
//    			return;
//    		};
    		if(iDynamic.getStatus() == Contants.SEND_DYNAMIC_STATUS.SUCCESS){
    		...
    		DynamicInfoItem infoItem = new DynamicInfoItem();
    		...
    		mDao.updateDate(Contants.SEND_DYNAMIC_STATUS.SENDING,date, item.getPid(),UserInfoUtil.getCurrentUserId());
    		SendDynamicThread task = new SendDynamicThread(mContext, infoItem);
    ...
    private void deleteDynamic (DynamicItem item) {
    	try {
    		DynamicDao mDao = new DynamicDao(this.mContext);
    		Integer result = mDao.deleteByPid(item.getPid(),UserInfoUtil.getCurrentUserId());
    		if (result > -1) {
    			mData.remove(item);
...
//				msg.what = 1;
//				msg.arg1 = pos;
//				msg.arg2 = RelationshipCircleAdapter.this.getCount(); 
//				msg.obj = reItem;
