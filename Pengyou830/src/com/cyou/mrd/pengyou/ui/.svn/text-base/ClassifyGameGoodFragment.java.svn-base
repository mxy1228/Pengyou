...
import com.cyou.mrd.pengyou.adapter.ClassifyGameGoodAdapter;
...
public class ClassifyGameGoodFragment extends BaseFragment implements
		OnClickListener {
	...
	private ClassifyGameGoodAdapter myGameAdapter;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		BehaviorInfo behaviorInfo = new BehaviorInfo(
				CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_CLASSIFY_GOOD_ID,
				CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_CLASSIFY_GOOD_NAME);
	...
	private void loadMyGameList() {
		...
		MyHttpConnect.getInstance().post2Json(
				HttpContants.NET.GAME_CLASSIFY_GOOD,
				params,
	...
	private void initData() {
		if (gameList == null || gameList.size() == 0) {
			...
			myGameAdapter = new ClassifyGameGoodAdapter(mActivity, gameList);
	...
	public ClassifyGameGoodFragment() {
		super();
	}

	public ClassifyGameGoodFragment(Activity mActivity2, String tid) {
		super(mActivity2);
	...
	private void updateListViewItem(GameItem item) {
		...
		if (view != null) {
			ClassifyGameGoodViewCache viewCache = (ClassifyGameGoodViewCache) view
					.getTag();
			if (viewCache == null) {
				log.e("视图为空!");
				return;
