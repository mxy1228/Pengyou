...
import com.cyou.mrd.pengyou.adapter.ClassifyGameHotAdapter;
...
public class ClassifyGameHotFragment extends BaseFragment implements
		OnClickListener {
	...
	private ClassifyGameHotAdapter myGameAdapter;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		BehaviorInfo behaviorInfo = new BehaviorInfo(
				CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_CLASSIFY_HOT_ID,
				CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_CLASSIFY_HOT_NAME);
	...
	private void loadMyGameList() {
		...
		MyHttpConnect.getInstance().post2Json(
				HttpContants.NET.GAME_CLASSIFY_HOT,
				params,
	...
	private void initData() {
		if (gameList == null || gameList.size() == 0) {
			...
			myGameAdapter = new ClassifyGameHotAdapter(mActivity, gameList);
	...
	public ClassifyGameHotFragment() {
		super();
	}

	public ClassifyGameHotFragment(Activity mActivity2, String tid) {
		super(mActivity2);
	...
	private void updateListViewItem(GameItem item) {
		...
		if (view != null) {
			ClassifyGameHotViewCache viewCache = (ClassifyGameHotViewCache) view
					.getTag();
			if (viewCache == null) {
				log.e("视图为空!");
				return;
