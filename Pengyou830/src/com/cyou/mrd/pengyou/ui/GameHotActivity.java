...
import com.cyou.mrd.pengyou.adapter.GameHotAdapter;
 ...
 *
 */
public class GameHotActivity extends CYBaseActivity implements OnClickListener {
    ...
    private List<GameItem> mGameHotList;
    ...
    protected void initView() {
        ...
        mHeaderTV.setText(R.string.game_hot_title);
        ...
        mPullListView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
                BehaviorInfo behaviorInfo = new BehaviorInfo(CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_HOTRANK_MOREGAME_GAMEDETAIL_ID,
                        CYSystemLogUtil.GAMESTORE.BTN_RECOMMEND_HOTRANK_MOREGAME_GAMEDETAIL_NAME);
                CYSystemLogUtil.behaviorLog(behaviorInfo);
                if (mGameHotList == null || mGameHotList.size() == 0
                        || position >= mGameHotList.size()) {
                    return;
                }
                GameItem item = mGameHotList.get(position-1);
                if (null == item) {
                    return;
                }
                Intent mIntent = new Intent();
                mIntent.setClass(GameHotActivity.this, GameCircleDetailActivity.class);
        ...
        mPullListView.setOnRefreshListener(new RefreshListener() {
            ...
            public void onRefresh() {
                ...
                CYSystemLogUtil.behaviorLog(behaviorInfo);
                if(Util.isNetworkConnected(GameHotActivity.this)){
                    requestGameNewData(true);
                }else{
                    Toast.makeText(GameHotActivity.this, GameHotActivity.this.getString(R.string.check_network), Toast.LENGTH_SHORT).show();
    ...
    protected void initData() {
        if (mGameHotList == null) {
            mGameHotList = new ArrayList<GameItem>();
        }
        if (mGameHotAdapter == null) {
            mGameHotAdapter = new GameHotAdapter(GameHotActivity.this, mGameHotList);
        }
        if (mPullListView != null) {
            mPullListView.setAdapter(mGameHotAdapter);
        }
        requestGameNewData(true);
    }
    /**
    ...
    private void requestGameNewData(final boolean refresh) {
        ...
        MyHttpConnect.getInstance().post(HttpContants.NET.GAME_RANK_WORLD, params,
                new JSONAsyncHttpResponseHandler(JSONAsyncHttpResponseHandler.RESULT_LIST,GameItem.class) {
                    ...
                    public void onSuccessForList(List list) {
                        ...
                        mGameHotList.addAll(list);
                        gameNewPageNum++;
                        mGameHotAdapter.notifyDataSetChanged();
    ...
    private void registReceiver(){
        ...
        GameHotActivity.this.registerReceiver(unstallAppReceiver, new IntentFilter(Contants.ACTION.UNINSTALL));
        GameHotActivity.this.registerReceiver(downloadAppReceiver, new IntentFilter(DownloadParam.UPDATE_PROGRESS_ACTION));
        GameHotActivity.this.registerReceiver(installAppReceiver, new IntentFilter(Contants.ACTION.GAME_INSTALL));
    ...
    private void unregistReceiver(){
        ...
        if (installAppReceiver != null) {
            GameHotActivity.this.unregisterReceiver(installAppReceiver);
            installAppReceiver = null;
        }
        if (unstallAppReceiver != null) {
            GameHotActivity.this.unregisterReceiver(unstallAppReceiver);
            unstallAppReceiver = null;
        }
        if (downloadAppReceiver != null) {
            GameHotActivity.this.unregisterReceiver(downloadAppReceiver);
    ...
    private void updateListViewItem(GameItem item) {
        ...
        int index;
        View view = null;
        if (mGameHotList == null || mGameHotList.isEmpty()) {
            return;
        }
        firstVisiblePosition = mPullListView.getFirstVisiblePosition();
        index = mGameHotList.indexOf(item)+1;
        if (index < 0) {
            return;
        }
        ...
        if (view != null) {
            ...
            if (Util.isInstallByread(item.getIdentifier())) {// 若已安装
                ...
                viewCache.getBtnDownloadGame().setTextColor(GameHotActivity.this.getResources().getColor(R.color.white));
            } else {
                ...
                if (downloadItem == null || TextUtils.isEmpty(downloadItem.getmPackageName())) {// 没下载
                    ...
                    if (downloadItem.getmState() == DownloadParam.C_STATE.DONE) {
                        ...
                        if (downloadItem.getmState() == DownloadParam.C_STATE.DOWNLOADING
                            ...
                            viewCache.getBtnDownloadGame().setTextColor( GameHotActivity.this.getResources().getColor(R.color.downloading_text));
                        } else {
                            viewCache.getBtnDownloadGame().setBackgroundResource(R.drawable.download_btn_xbg);
                            viewCache.getBtnDownloadGame().setText(R.string.game_download);
                            viewCache.getBtnDownloadGame().setTextColor(GameHotActivity.this.getResources().getColor(R.color.white));
    ...
    private GameItem getGameItemByPackage(String packageName) {
        ...
        if (null == mGameHotList || mGameHotList.isEmpty()) {
            return mGameItem;
        }
        for (GameItem item : mGameHotList) {
