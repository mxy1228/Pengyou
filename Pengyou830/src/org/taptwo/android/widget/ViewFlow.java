...
//				if (startX == ev.getX()) {
//					if (0 == mCurrentScreen
//							|| mCurrentScreen == this.getAdapter().getCount() - 1) {
						getParent().requestDisallowInterceptTouchEvent(true);
//					}
//				}
				// 里层viewpager已经是最后一页，此时继续向右滑（手指从右往左滑）
//				else if (startX > ev.getX()) {
//					if (mCurrentScreen == this.getAdapter().getCount() - 1) {
//						getParent().requestDisallowInterceptTouchEvent(false);
//					}
//				}
...
//					getParent().requestDisallowInterceptTouchEvent(true);
//				}
				final int xDiff = (int) Math.abs(startX - mLastMotionX);
				...
				if (mTouchState == TOUCH_STATE_SCROLLING) {
					// Scroll to follow the motion event
					final int deltaX = (int) (mLastMotionX - startX);
					mLastMotionX = startX;
