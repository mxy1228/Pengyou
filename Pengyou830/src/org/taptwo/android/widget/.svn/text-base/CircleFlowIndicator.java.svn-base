...
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	private Paint mPaintActive = new Paint(Paint.ANTI_ALIAS_FLAG);
	...
	private boolean isSetImg;// 底部滑动的原点是否设置图片
	private int activeResId;// 活动中时资源ID
	private int inActiveResId;// 非活动中资源ID
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		bitPic = BitmapFactory.decodeResource(
				CyouApplication.mAppContext.getResources(),
				R.drawable.gamestore_senamir_active);
		bitInactivePic = BitmapFactory.decodeResource(
				CyouApplication.mAppContext.getResources(),
				R.drawable.gamestore_senamir_inactive);
		...
		isSetImg = a.getBoolean(R.styleable.CircleFlowIndicator_isimgbg, false);
		// Retrieve the radius
		radius = a.getDimension(R.styleable.CircleFlowIndicator_radius, 4.0f);
		if (isSetImg) {
			radius = 10;
			if (null != CyouApplication.getMainActiv()) {
				if (Util.getScreenWidthSize(CyouApplication.getMainActiv()) > 480) {
					radius = 14;
				}
				if(Util.getScreenWidthSize(CyouApplication.getMainActiv()) > 1000){
					radius = 20;
				}
			}
		}
		...
		if (isSetImg) {
			...
			// if (Util.getScreenWidthSize(CyouApplication.getMainActiv()) >
			// 480) {
	...
	public void initBgImg() {
		try {
			...
			BitmapShader bitmapShader = new BitmapShader(bitPic,
					TileMode.REPEAT, TileMode.MIRROR);
			mPaintActive.setShader(bitmapShader);
			BitmapShader bitmapInactiveShader = new BitmapShader(
					bitInactivePic, TileMode.REPEAT, TileMode.MIRROR);
			mPaintInactive.setShader(bitmapInactiveShader);
		} catch (Exception e) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			if (isSetImg) {
				canvas.drawBitmap(bitInactivePic, leftPadding + radius
						+ (iLoop * circleSeparation) + centeringOffset,
						getPaddingTop() + (radius / 2), mPaintInactive);
			} else {
		...
		if (isSetImg) {
			canvas.drawBitmap(bitPic, leftPadding + radius + cx
					+ centeringOffset, getPaddingTop() + (radius / 2),
					mPaintActive);
		} else {
